;(function () {
    ...
    Sy.shareRead(sid).then(function (r) {
      ...
      if (doc.ov && (doc.ov.ink || (doc.ov.shapes && doc.ov.shapes.length)) &&
        ...
        overlay = GardenNotesOverlay.mount({
          ...
          stage: document.getElementById('na-zoom'),
          bound: !isBoard,
          sheet: host,
          data: doc.ov,
          onChange: function () {}
  ...
  function renderQuota() {
    ...
    St.quota().then(function (q) {
      ...
      els.quota.setAttribute('aria-label', L('المساحة المستعملة: ', 'Storage used: ') + full);
  ...
  function renderList() {
    ...
    els.items.innerHTML = list.length
  ...
  function saveState(s, txt) {
    /*@3.NOAJ.105*/
    if (els.saveDot) els.saveDot.setAttribute('data-s', s || '');
    if (!els.save) return;
  ...
  function setDocActions(on) {
    ...
    ['naDel', 'naPin', 'naMove', 'naPage', 'naPdf'].forEach(function (k) {
  ...
  var MORE = [
    ...
    { id: 'na-export',   icon: 'fa-file-export', ar: 'تصدير',        en: 'Export' },
    { act: 'md',         icon: 'fa-file-import', ar: 'استيراد ماركداون', en: 'Import Markdown' },
  ...
  function openMore(anchor) {
    ...
    m.innerHTML = MORE.map(function (it) {
      ...
      return '<button type="button" class="ne-menu-i' + (it.danger ? ' ne-menu-i--danger' : '') +
        ...
        (it.act ? ' data-mact="' + it.act + '"' : '') +
        ((it.id && src && src.disabled) || (it.act && !ed) ? ' disabled' : '') + '>' +
    ...
    m.addEventListener('click', function (e) {
      ...
      closeMore();
      if (act === 'md') { importMarkdown(); return; }
  ...
  function openNote(id, opts) {
    ...
    p.then(function (row) {
      ...
      ed = GardenNotesEditor.mount(host, doc, {
        ...
        onSave: function (d) { persist(id, d); },
  ...
  function stageZoom() {
    ...
    var z = st.getBoundingClientRect().width / st.offsetWidth;
...
/*@3.NOAJ.31*/
  function contentBottom() {
    ...
    if (root) {
      ...
      for (i = 0; i < kids.length; i++) {
        if (kids[i].classList && kids[i].classList.contains('ne-tail')) continue;
        used = Math.max(used, (kids[i].getBoundingClientRect().bottom - sTop) / z);
      }
    }
    /*@3.NOAJ.24*/
    var d = ed && ed.doc;
  ...
  function updatePgNav() {
    ...
    var on = !!sheet && pages > 1 && !isBoard;
    els.pgnavT.textContent = pgCurPage() + ' / ' + pages;
    els.pgnav.hidden = !on;
  }
  ...
  function buildPgNav() {
    ...
    nav.addEventListener('click', function (e) {
      ...
      if (go) { pgGoto(pgCurPage() + Number(go.getAttribute('data-go'))); return; }
...
/*@3.NOAJ.32*/
  ...
  function syncIcons() {
    var f = document.getElementById('na-go-folders');
    var l = document.getElementById('na-go-list');
    if (f) f.setAttribute('aria-pressed', (S.panel && S.acc === 'folders') ? 'true' : 'false');
    if (l) l.setAttribute('aria-pressed', (S.panel && S.acc === 'list') ? 'true' : 'false');
  }

  function showPanel(which) {
    /*@3.NOAJ.103*/
    if (isPhone()) { setMob('list'); setAcc(which); setPanel(true); return; }
  ...
  function openCtx(x, y, uid) {
    ...
    m.addEventListener('click', function (e) {
      ...
      if (!b) return;
      closeCtx();
      ctxAct(b.getAttribute('data-cact'), uid);
    });
    setTimeout(function () {
  ...
  function onDocKey(e) {
    if (!ed || !edId || e.defaultPrevented) return;
    var t = e.target;
    var tag = t && t.tagName;
  ...
  function init() {
    ...
    document.addEventListener('wheel', function (e) {
      if (!ed || !edId || !(e.ctrlKey || e.metaKey)) return;
      if (!e.target.closest || !e.target.closest('.na-doc-body')) return;
      e.preventDefault();
      stepFs(e.deltaY < 0 ? 1 : -1);
    }, { passive: false });
    ...
    buildPageDlg();
    bindPageSliders();
    var impBtn = document.getElementById('na-imp-btn');
    ...
    if (lDlg) {
      ...
      var applyLink = function () {
        ...
        var v = raw ? (B0 && B0.normUrl ? B0.normUrl(raw) : raw) : '';
        if (raw && !v) {
    ...
    if (nw) nw.addEventListener('click', function () { createNote(); });
    var nw2 = document.getElementById('na-new-2');
    if (nw2) nw2.addEventListener('click', function () { createNote(); });
    /*@3.NOAJ.71*/
    var nb = document.getElementById('na-new-board');
    if (nb) nb.addEventListener('click', function () { createNote('board'); });
    ...
    var gf = document.getElementById('na-go-folders');
    if (gf) gf.addEventListener('click', function () { showPanel('folders'); });
    var gl = document.getElementById('na-go-list');
    if (gl) gl.addEventListener('click', function () { showPanel('list'); });
    ...
    if (cDlg) {
      ...
      cDlg.addEventListener('cancel', function () { pendDelete = null; pendMany = null; pendFolder = null; });
    ...
    window.addEventListener('garden:syncCompleted', function () { reload({ keepOpen: true }); });
    window.addEventListener('garden:notesReconciled', function () { reload({ keepOpen: true }); });
    ...
    try {
      ...
      if (wanted && idxFind(wanted)) openNote(wanted);
      else if (qs.get('new') === '1') createNote();
    } catch (e) {}

    if (window.GardenNotesSync) {
      var live = idxRead().map(function (x) { return x.id; });
      window.GardenNotesSync.reconcile({ liveIds: live });
    }
  }
